...
function filterEnum(array, selectedColors) {
  ...
  return array.filter(item => selectedColors.includes(item.eyeColor));
}

function filter(query, array, keyData) {
  return array.filter(
    item => item[keyData].toLowerCase().indexOf(query.toLowerCase()) + 1
  );
}
...
function sortNumber(direction, array, keyData) {
  return array.sort((item1, item2) =>
    direction
      ? item1[keyData] - item2[keyData]
      : item2[keyData] - item1[keyData]
  );
}

function filterActive(checked, array, array2) {
  return checked ? array.filter(item => item.isActive) : array2;
}
